...
describe(Volunteer) do
    ...
    describe('#name') do
      it("initializes the system to add volunteers to it") do
        test_volunteer = Volunteer.new({:name => "Lille Skutt", :id => nil, :phone => "1234", :availability => "Mondays"})
    ...
    describe("#phone") do
     it("lets you read the volunteer phone #") do
       test_volunteer = Volunteer.new({:name => "Lille Skutt", :id => nil, :phone => "1234", :availability => "Mondays"})
       expect(test_volunteer.phone()).to(eq("1234"))
     end
   end
    ...
    describe("#:availability") do
     it("lets you read the volunteer ID") do
       test_volunteer = Volunteer.new({:name => "Lille Skutt", :id => nil, :phone => "1234", :availability => "Mondays"})
       expect(test_volunteer.availability()).to(eq("Mondays"))
     end
   end
   ...
   describe("#save") do
    ...
    describe('#==') do
      it('is the same volunteer if it has the same id') do
        test_volunteer = Volunteer.new({:name => "Lille Skutt", :id => nil, :phone => "1234", :availability => "Mondays"})
        test_volunteer2 = Volunteer.new({:name => "Lille Skutt", :id => nil, :phone => "1234", :availability => "Mondays"})
    ...
    describe("#update") do
      it("lets the volunteer be updated") do
        test_volunteer = Volunteer.new({:name => "Lille Skutt", :id => nil, :phone => "1234", :availability => "Mondays"})
        test_volunteer.save()
        test_volunteer.update({:name => "Lilla Skutt", :id => nil, :phone => "1234", :availability => "Mondays"})
  ...
  describe(".find") do
    it("returns a volunteer by name") do
      test_volunteer = Volunteer.new({:name => "Lille Skutt", :id => nil, :phone => "1234", :availability => "Mondays"})
      test_volunteer.save()
      test_volunteer2 = Volunteer.new({:name => "Bamse", :id => nil, :phone => "5678", :availability => "Mondays"})
      test_volunteer2.save()
